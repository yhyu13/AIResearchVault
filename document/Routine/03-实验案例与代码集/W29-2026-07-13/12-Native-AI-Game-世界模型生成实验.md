...
class ProceduralWorldGenerator:
    """
    程序化世界生成器 —— 用算法创造无限多样化的世界
    
    初学者要点：
    - 核心思想：用数学（噪声函数）而非手工制作来生成内容
    - 关键技术：
      1. fBm（分形布朗运动）：叠加多层噪声生成自然地形
      2. 阈值分类：连续高度 → 离散地形类型（水域/平原/森林等）
      3. 规则放置：基于地形类型放置资源、建筑、NPC
    - 与神经网络生成的区别：确定性（相同 seed 产生相同世界）、可解释、零训练成本
    """
...
class WorldModel:
    """
    世界模型 —— 预测"如果这样做，世界会怎样变化"
    
    初学者要点：
    - 核心思想：世界模型 = 游戏引擎的"物理定律"
    - 三个核心预测函数：
      1. predict_move：预测移动后的位置（考虑碰撞）
      2. predict_gather：预测能否采集到资源
      3. predict_build：预测能否在此建造
    - 与真实游戏引擎的区别：这里是规则驱动，Genie 是神经网络驱动
    - 世界模型让 Agent 可以"想象"行动后果，而不必真的执行
    """
...
class Player(Entity):
    ...
    初学者要点：
    - 拥有背包（Inventory）存储采集的资源
    - 视野系统（vision_range）：只能看到周围 8 格，模拟"迷雾"
    - 与 NPC 的区别：玩家由人类输入控制，NPC 由 AI 控制
    """
    pos: Position
    name: str = "Player"
    symbol: str = 'P'
...
class NPC(Entity):
    """
    NPC 实体 —— 由简单 AI 控制的非玩家角色
    
    初学者要点：
    - 每个 NPC 有角色（村民/商人/守卫/探索者），决定行为模式
    - 简单 AI 逻辑：选择目标 → 向目标移动 → 能量耗尽则重置
    - 不同角色的目标选择策略不同（村民附近徘徊，探索者远行）
    - 这是"最简可行 AI"，真实游戏会用行为树或强化学习
    """
    pos: Position
    role: NPCRole
    name: str
...
class WorldRenderer:
    ...
    初学者要点：
    - 使用 Matplotlib 绘制 2D 网格世界
    - 渲染层级：地形 → 资源 → 建筑 → NPC → 玩家 → 视野范围
    - 支持两种模式：图形界面（matplotlib）和 终端 ASCII
    - 可以保存帧为图片，用于制作世界演化动画
    """
...
class GameEngine:
    ...
    - 两种运行模式：
      1. 交互模式：玩家按键控制，实时渲染
      2. 模拟模式：自动运行，观察世界演化
    - 输入映射：w/a/s/d=移动, g=采集, b/1-4=建造, t=推进时间
    - 每次玩家行动后自动 tick()，让 NPC 也行动一步
    """
...
Native AI Game 融合了两条技术路径：程序化生成（创造世界）和世界模型（管理交互）。这些指标帮助我们评估：
- 生成的世界是否丰富多样？
- 世界模型是否正确预测交互后果？
- NPC 是否能有效行动？
...
|-----|------|----------|--------|---------|
| **地形多样性** | 5种地形类型的分布均衡度 | 避免世界过于单调 | 各类型占比 15-30% | 调整 `_fbm()` 的 persistence 和阈值 |
| **生成时间** | 完整世界生成耗时 | 衡量实时生成的可行性 | < 5秒 | 减少 world_size 或 octaves |
| **可达区域比例** | 可行走格子 / 总格子 | 确保玩家有足够的探索空间 | > 40% | 调整地形阈值，减少水域/山地 |
| **NPC 效率** | NPC 成功移动率 | 衡量 AI 行为合理性 | > 80% | 改进 `_choose_target()` 的目标选择 |
| **资源平衡** | 各资源类型的分布合理性 | 确保游戏经济系统平衡 | 符合地形规则 | 调整 generate_resources() 的概率 |
| **聚落数量** | 生成的建筑群数量 | 衡量社会结构的形成 | 3-8 个 | 调整 generate_buildings() 的概率 |

### 指标之间的关系

```
地形多样性 → 资源分布 → NPC 行为 → 玩家体验
     ↑
生成时间 ← world_size × octaves
```

**关键洞察**：
- 地形多样性低 → 世界看起来"假"，玩家容易厌倦
- 可达区域 < 30% → 玩家可能被困在出生点，游戏体验差
- NPC 效率低 → NPC 频繁卡住，世界缺乏"生机"
- 聚落数量 = 0 → 没有社会结构，世界像荒野而非文明
...
| 场景 | world_size | octaves | NPC数量 | 用途 |
|-----|-----------|---------|--------|------|
| 快速测试 | 32 | 4 | 5 | 秒级生成，验证流程 |
| 标准体验 | 64 | 6 | 15 | 平衡性能与内容（推荐） |
| 大型世界 | 128 | 8 | 30 | 丰富探索体验 |
| 性能测试 | 256 | 6 | 50 | 测试生成性能边界 |
| 极简演示 | 16 | 3 | 3 | 教学演示用 |

### 初学者调试清单

- [ ] **如果生成时间过长**：减少 `world_size` 或 `octaves`
- [ ] **如果地形太单一**：调整 `_fbm()` 的 `persistence` 参数（越高越粗糙）
- [ ] **如果 NPC 卡住**：检查 `is_valid_position()` 的碰撞逻辑
- [ ] **如果资源太少**：调高 `generate_resources()` 的概率阈值
- [ ] **如果没有聚落**：检查 `generate_buildings()` 的水域邻接条件
- [ ] **如果玩家出生在不可行走位置**：检查 `_spawn_player()` 的循环次数
